...
def migrate(apps, schema_editor):
    ...
    LinkedInAPITextPage = apps.get_model("homepage", "LinkedInAPITextPage")
    ...
    TextPage.objects.using(db_alias).create(
        ...
        content="".join(open(os.path.join(
            settings.HOMEPAGE_MD_FOLDER, 'main-page.md')).readlines()),
        order=1,
        slug='main-page')
    TextPage.objects.using(db_alias).create(
        title='About Me',
        content="".join(open(os.path.join(
            settings.HOMEPAGE_MD_FOLDER, 'about-me.md')).readlines()),
        order=2,
        slug='about-me')

    LinkedInAPITextPage.objects.using(db_alias).create(
        title='Career Bio',
        content="".join(open(os.path.join(
    ...
    MarkdownText.objects.using(db_alias).create(
        ...
        content="".join(open(os.path.join(
            settings.HOMEPAGE_MD_FOLDER, 'header.md')).readlines()))
